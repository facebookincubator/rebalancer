...
class HiGHSRelation : public RelationImpl {
 public:
  explicit HiGHSRelation(
      const HiGHSExpression& expr,
      Relation::ConstantType sense);

  const HiGHSExpression& getExpression() const;
  Relation::ConstantType getType() const;

 private:
  HiGHSExpression expression_;
