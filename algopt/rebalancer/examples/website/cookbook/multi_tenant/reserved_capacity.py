...
"""
Multi-Tenant with Reserved Capacity

This variation demonstrates how to reserve specific servers for high-priority
tenants. Premium tenants get dedicated servers (0-5), while other tenants
can use the remaining servers (6-19).

Key Feature: AvoidAssignmentsSpec to block non-premium workloads from premium servers
"""
...
def allocate_multi_tenant_with_reserved_capacity():
    ...
    solver.addConstraint(
        ConstraintSpecs(
            avoidAssignmentsSpec=AvoidAssignmentsSpec(
                ...
                assignments=[
                    AvoidAssignment(
                        object=w, scopeItems=[f"server{i}" for i in range(6)]
                    )
    ...
    print("\nReserved capacity verification:")
    premium_servers = [f"server{i}" for i in range(6)]
    for server in premium_servers:
        workloads = solution.assignment.get(server, [])
        non_premium = [w for w in workloads if w not in premium_workloads]
        if non_premium:
            print(f"  X {server} has non-premium workloads: {non_premium}")
        else:
            print(
                f"  OK {server} reserved for premium (has {len(workloads)} premium workloads)"
            )
...
if __name__ == "__main__":
    allocate_multi_tenant_with_reserved_capacity()
