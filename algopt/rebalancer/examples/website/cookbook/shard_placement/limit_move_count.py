...
"""
Database Shard Placement with Limited Move Count

This variation limits the number of shards moved per round, useful for
gradual rebalancing to minimize disruption to production systems.
"""
...
def place_database_shards():
    ...
    # Add move count dimension
    solver.addObjectDimension("count", {f"shard{i}": 1.0 for i in range(50)})
    ...
    # variation_start
    # Strongly minimize number of moves
    solver.addGoal(
        GoalSpecs(
            minimizeMovementSpec=MinimizeMovementSpec(
                name="limit-move-count",
                scope="server",
                dimension="count",
            )
        ),
        weight=1.0,  # Equal to balance goals
    )
    # variation_end
