...
"""
Database Shard Placement with Rack Diversity

This variation adds rack diversity constraints to ensure shards are spread
across racks for fault tolerance. Useful when you want to protect against
rack-level failures.
"""
    ...
    solver.addGoal(
        GoalSpecs(
            balanceSpec=BalanceSpec(
                name="balance-storage-across-racks",
                scope="rack",
                dimension="storage_gb",
                formula=BalanceSpecFormula.LEGACY,
            )
        ),
        weight=0.5,  # Lower priority than server-level balance
    )
    # variation_end
