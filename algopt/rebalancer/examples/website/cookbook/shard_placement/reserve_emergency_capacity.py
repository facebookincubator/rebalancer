...
"""
Database Shard Placement with Reserved Emergency Capacity

This variation adds soft upper bounds to keep some capacity free for emergency
growth. Useful when you want to ensure headroom for unexpected load spikes.
"""
...
def place_database_shards():
    ...
    # variation_start
    # Add soft upper bound (90% of capacity)
    solver.addGoal(
        GoalSpecs(
            balanceSpec=BalanceSpec(
                ...
                scope="server",
                dimension="storage_gb",
                upperBound=0.9,  # Keep under 90% utilization
                boundType=BalanceSpecBoundType.RELATIVE_UTIL,
            )
        ),
        weight=0.5,
    )
