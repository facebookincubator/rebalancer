...
Demonstrates enforcing server-level diversity in addition to rack and datacenter
diversity. This ensures that no two replicas of the same shard are on the same server.

This is a fundamental constraint for most distributed systems - you never want
multiple replicas of the same data on the same physical machine, as a single
server failure would then impact multiple replicas.
"""
...
def place_replicas_with_server_diversity():
    """Place replicas with server-level diversity constraint."""
...
if __name__ == "__main__":
    place_replicas_with_server_diversity()
