...
def place_replicas_with_proportional_dc():
    """Place replicas proportionally to datacenter capacity."""
    ...
    solver.addGoal(
        GoalSpecs(
            groupCountSpec=GroupCountSpec(
                name="proportional-dc-placement",
                scope="datacenter",
                partitionName="shard",
                bound=GroupCountSpecBound.MAX,
                limit=Limit(
                    type=LimitType.RELATIVE,
                    relativeLimitPercentage=0.67,  # Max 67% in one DC
                ),
            )
        ),
        weight=0.5,
...
if __name__ == "__main__":
    place_replicas_with_proportional_dc()
