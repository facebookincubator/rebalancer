...
/**
 * Proportional DC Placement Variation
 *
 * Demonstrates placing replicas proportionally to datacenter capacity using
 * relative limits. This is useful when datacenters have different sizes and
 * you want to distribute load proportionally rather than evenly.
 *
 * For example, if DC1 has 2x the capacity of DC2, you'd want roughly 2/3 of
 * replicas in DC1 and 1/3 in DC2, rather than forcing them to be equal.
 */
...
int main(int argc, char* argv[]) {
  ...
  // variation_start
  // 2/3 in DC1, 1/3 in DC2 (if DC1 is larger)
  GroupCountSpec proportionalDc;
  proportionalDc.name() = "proportional-dc-placement";
  proportionalDc.scope() = "datacenter";
  proportionalDc.partitionName() = "shard";
  proportionalDc.bound() = GroupCountSpecBound::MAX;
  proportionalDc.limit() = Limit();
  proportionalDc.limit()->type() = LimitType::RELATIVE;
  proportionalDc.limit()->globalLimit() = 0.67; // Max 67% in one DC
  solver.addGoal(proportionalDc, 0.5);
  // variation_end
