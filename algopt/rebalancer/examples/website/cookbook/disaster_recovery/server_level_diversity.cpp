...
/**
 * Server-Level Diversity Variation
 *
 * Demonstrates enforcing server-level diversity in addition to rack and
 * datacenter diversity. This ensures that no two replicas of the same shard are
 * on the same server.
 *
 * This is a fundamental constraint for most distributed systems - you never
 * want multiple replicas of the same data on the same physical machine, as a
 * single server failure would then impact multiple replicas.
 */
...
int main(int argc, char* argv[]) {
  ...
  solver.addPartition("shard", std::move(shard_partition));
  ...
  solver.addObjectDimension("storage_gb", std::move(replica_storage));
  ...
  // Hard constraints
  // variation_start
  GroupCountSpec serverDiversity;
  serverDiversity.name() = "server-diversity";
  serverDiversity.scope() = "server";
  serverDiversity.partitionName() = "shard";
  serverDiversity.bound() = GroupCountSpecBound::MAX;
  serverDiversity.limit() = Limit();
  serverDiversity.limit()->type() = LimitType::ABSOLUTE;
  serverDiversity.limit()->globalLimit() = 1;
  solver.addConstraint(std::move(serverDiversity));
  // variation_end
  ...
  solver.addConstraint(std::move(rackDiversity));
  ...
  solver.addConstraint(std::move(dcDiversity));
  ...
  solver.addObjectDimension("count", std::move(replica_count));
  ...
  solver.addGoal(std::move(balanceCount), 1.0);
  ...
  solver.addGoal(std::move(balanceStorage), 1.0);
