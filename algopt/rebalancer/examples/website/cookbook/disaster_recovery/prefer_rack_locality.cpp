 ...
 *
 * Demonstrates adding a secondary goal to prefer keeping replicas of the same
 * shard closer together (in fewer racks) while still maintaining diversity
 * requirements.
 *
 * This is useful when network latency between racks matters and you want to
 * balance fault tolerance with performance by preferring replicas to be in
 * fewer racks (but still respecting the hard constraint of max 1 replica per
 * rack per shard).
 */
...
int main(int argc, char* argv[]) {
  ...
  rackLocality.name() = "prefer-rack-locality";
  rackLocality.scope() = "rack";
  rackLocality.partitionName() = "shard";
  ...
  solver.addGoal(rackLocality, 0.3); // Lower priority
  // variation_end
