...
LogicalAndSpecBuilder::LogicalAndSpecBuilder(
    std::shared_ptr<const Universe> universe,
    interface::LogicalAndSpec spec)
    : SpecBuilder(std::move(universe)), spec_(std::move(spec)) {}

folly::coro::Task<std::vector<ConstraintInfo>>
LogicalAndSpecBuilder::constraints(ExpressionBuilder& expressionBuilder) const {
...
folly::coro::Task<ExprPtr> LogicalAndSpecBuilder::goalCoro(
  ...
  throw std::runtime_error("LogicalAndSpec not supported as a goal");
}

std::string LogicalAndSpecBuilder::description() const {
  ...
  return fmt::format(
      "ANDed generic specs: {}", folly::join(" AND ", descriptions));
}

SpecParameters LogicalAndSpecBuilder::getSpecInfo() const {
