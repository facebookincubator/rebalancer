...
CO_TEST_P(CapacityWithGroupPresenceSpecBuilderTest, WithMinBound) {
  interface::CapacityWithGroupPresenceSpec spec;
  spec.dimension() = "replicaCount";
  spec.partition() = "tenantTrafficObjects";
  spec.scope() = "region";
  ...
  auto& scopeItemLimits = *spec.scopeItemToLimit();
  scopeItemLimits.type() = interface::LimitType::ABSOLUTE;
  scopeItemLimits.globalLimit() = 6.5;
...

  {
    ...
    const CapacityWithGroupPresenceSpecBuilder goalSpecBuilder(
        universe, spec, true);
    ...
    switch (GetParam()) {
      case interface::CapacityWithGroupPresenceUsageIntent::PER_SCOPE_ITEM: {
        // 2 constraints (one per scope item)
        // MIN-bound: penalty = (limit - continuousUtil), scaled.
        initialExpectedInfo.constraintAndPenaltyValues = {
            // region1
            {.constraintValue = 6.5 - (4.0 + 2.0),
             .penaltyValue = (6.5 - (3.18 + 1)) * kNormPerScopeItem},
            // region2
            {.constraintValue = 6.5 - (3.0 + 2.0),
             .penaltyValue = (6.5 - (1.5 + 1.995)) * kNormPerScopeItem},
        };

        initialExpectedInfo.goalValue = (6.5 - (4.0 + 2.0)) +
            (6.5 - (3.18 + 1)) * kNormPerScopeItem + (6.5 - (3.0 + 2.0)) +
            (6.5 - (1.5 + 1.995)) * kNormPerScopeItem;
        break;
      }
      case interface::CapacityWithGroupPresenceUsageIntent::
          PER_GROUP_AND_SCOPE_ITEM: {
        ...
        initialExpectedInfo.constraintAndPenaltyValues = {
            ...
            {.constraintValue = 6.5 - 4.0,
             .penaltyValue = (6.5 - 3.18) * kNormTenant1},
            // tenant2-region1
            {.constraintValue = 6.5 - 2.0,
             .penaltyValue = (6.5 - 1.0) * kNormTenant2},
            // tenant1-region2
            {.constraintValue = 6.5 - 3.0,
             .penaltyValue = (6.5 - 1.5) * kNormTenant1},
            // tenant2-region2
            {.constraintValue = 6.5 - 2.0,
             .penaltyValue = (6.5 - 1.995) * kNormTenant2},
        };

        initialExpectedInfo.goalValue =
            ((6.5 - 4.0) + (6.5 - 3.18) * kNormTenant1) +
            ((6.5 - 2.0) + (6.5 - 1.0) * kNormTenant2) +
            ((6.5 - 3.0) + (6.5 - 1.5) * kNormTenant1) +
            ((6.5 - 2.0) + (6.5 - 1.995) * kNormTenant2);
        break;
      }
    }
    VERIFY_CONSTRAINT_COMPONENTS_AND_GOAL_VALUES(
        initialExpectedInfo, components, goalExpr, initial);
    ...
    switch (GetParam()) {
      case interface::CapacityWithGroupPresenceUsageIntent::PER_SCOPE_ITEM: {
        ...
        deltaExpectedInfo.constraintAndPenaltyValues = {
            // region1 component
            {.constraintValue = 6.5 - (5.0 + 2.0),
             .penaltyValue = (6.5 - (4.18 + 1.0)) * kNormPerScopeItem},
            // region2 component
            {.constraintValue = 6.5 - (3.0 + 2.0),
             .penaltyValue = (6.5 - (0.5 + 1.995)) * kNormPerScopeItem},
        };

        // only region2 broken
        deltaExpectedInfo.goalValue =
            (6.5 - (3.0 + 2.0)) + (6.5 - (0.5 + 1.995)) * kNormPerScopeItem;
        break;
      }
      case interface::CapacityWithGroupPresenceUsageIntent::
          PER_GROUP_AND_SCOPE_ITEM: {
        ...
        deltaExpectedInfo.constraintAndPenaltyValues = {
            // tenant1-region1
            {.constraintValue = 6.5 - 5.0,
             .penaltyValue = (6.5 - 4.18) * kNormTenant1},
            // tenant2-region1
            {.constraintValue = 6.5 - 2.0,
             .penaltyValue = (6.5 - 1.0) * kNormTenant2},
            // tenant1-region2
            {.constraintValue = 6.5 - 3.0,
             .penaltyValue = (6.5 - 0.5) * kNormTenant1},
            // tenant2-region2
            {.constraintValue = 6.5 - 2.0,
             .penaltyValue = (6.5 - 1.995) * kNormTenant2},
        };
        ...
        deltaExpectedInfo.goalValue =
            ((6.5 - 5.0) + (6.5 - 4.18) * kNormTenant1) +
            ((6.5 - 2.0) + (6.5 - 1.0) * kNormTenant2) +
            ((6.5 - 3.0) + (6.5 - 0.5) * kNormTenant1) +
            ((6.5 - 2.0) + (6.5 - 1.995) * kNormTenant2);
        break;
      }
    }
    VERIFY_CONSTRAINT_COMPONENTS_AND_GOAL_VALUES(
        deltaExpectedInfo, components, goalExpr, delta);
  }
}

CO_TEST_P(
    CapacityWithGroupPresenceSpecBuilderTest,
    WithMinBoundAndWithoutRoundUpAndGroupToPresenceWeight) {
  interface::CapacityWithGroupPresenceSpec spec;
  spec.dimension() = "replicaCount";
  spec.partition() = "tenantTrafficObjects";
  spec.scope() = "region";
  ...
  scopeItemLimits.type() = interface::LimitType::ABSOLUTE;
  scopeItemLimits.globalLimit() = 6.5;
  scopeItemLimits.scopeItemLimits() = {{"region1", 4.58}};

  auto& groupToPresenceWeight = *spec.groupToPresenceWeight();
  groupToPresenceWeight.globalLimit() = 0.0;
  ...
  const CapacityWithGroupPresenceSpecBuilder localSearchGoalSpecBuilder(
      universe, spec, true);
  auto goalExpr = co_await localSearchGoalSpecBuilder.goalCoro(builder);
  ...
  switch (GetParam()) {
    case interface::CapacityWithGroupPresenceUsageIntent::PER_SCOPE_ITEM: {
      // 2 constraints (one per scope item)
      initialExpectedInfo.constraintAndPenaltyValues = {
          // Region1 component (limit of region1 = 4.58)
          {.constraintValue = 4.58 - (3.18 + 1.0),
           .penaltyValue = (4.58 - (3.18 + 1.0)) * kNormPerScopeItemNoRoundUp},
          // Region2 component (limit of region2 = 6.5)
          {.constraintValue = 6.5 - (1.5 + 1.995),
           .penaltyValue = (6.5 - (1.5 + 1.995)) * kNormPerScopeItemNoRoundUp},
      };

      initialExpectedInfo.goalValue = (4.58 - (3.18 + 1.0)) +
          (4.58 - (3.18 + 1.0)) * kNormPerScopeItemNoRoundUp +
          (6.5 - (1.5 + 1.995)) +
          (6.5 - (1.5 + 1.995)) * kNormPerScopeItemNoRoundUp;
      break;
    }
    case interface::CapacityWithGroupPresenceUsageIntent::
        PER_GROUP_AND_SCOPE_ITEM: {
      ...
      initialExpectedInfo.constraintAndPenaltyValues = {
          ...
          {.constraintValue = 4.58 - 3.18,
           .penaltyValue = (4.58 - 3.18) * kNormTenant1NoRoundUp},
          // tenant2-region1
          {.constraintValue = 4.58 - 1.0,
           .penaltyValue = (4.58 - 1.0) * kNormTenant2NoRoundUp},
          // tenant1-region2
          {.constraintValue = 6.5 - 1.5,
           .penaltyValue = (6.5 - 1.5) * kNormTenant1NoRoundUp},
          // tenant2-region2
          {.constraintValue = 6.5 - 1.995,
           .penaltyValue = (6.5 - 1.995) * kNormTenant2NoRoundUp},
      };

      initialExpectedInfo.goalValue =
          ((4.58 - 3.18) + (4.58 - 3.18) * kNormTenant1NoRoundUp) +
          ((4.58 - 1.0) + (4.58 - 1.0) * kNormTenant2NoRoundUp) +
          ((6.5 - 1.5) + (6.5 - 1.5) * kNormTenant1NoRoundUp) +
          ((6.5 - 1.995) + (6.5 - 1.995) * kNormTenant2NoRoundUp);
      break;
    }
  }
  VERIFY_CONSTRAINT_COMPONENTS_AND_GOAL_VALUES(
  ...
  // expect goal value to decrease when a new object is moved from outside
  // region1 to region1
  auto delta1 = deltaFromInitial({
      {"trafficObject10", "host1"},
  });
  ExpectedInfo delta1ExpectedInfo;
  switch (GetParam()) {
    case interface::CapacityWithGroupPresenceUsageIntent::PER_SCOPE_ITEM: {
      // 2 constraints (one per scope item)
      // trafficObject10 (value 0.3) moves from outside to region1
      delta1ExpectedInfo.constraintAndPenaltyValues = {
          // Region1 component
          {.constraintValue = 4.58 - (3.48 + 1.0),
           .penaltyValue = (4.58 - (3.48 + 1.0)) * kNormPerScopeItemNoRoundUp},
          // Region2 component
          {.constraintValue = 6.5 - (1.5 + 1.995),
           .penaltyValue = (6.5 - (1.5 + 1.995)) * kNormPerScopeItemNoRoundUp},
      };

      delta1ExpectedInfo.goalValue = (4.58 - (3.48 + 1.0)) +
          (4.58 - (3.48 + 1.0)) * kNormPerScopeItemNoRoundUp +
          (6.5 - (1.5 + 1.995)) +
          (6.5 - (1.5 + 1.995)) * kNormPerScopeItemNoRoundUp;
      break;
    }
    case interface::CapacityWithGroupPresenceUsageIntent::
      ...
      delta1ExpectedInfo.constraintAndPenaltyValues = {
          ...
          {.constraintValue = 4.58 - 3.48,
           .penaltyValue = (4.58 - 3.48) * kNormTenant1NoRoundUp},
          // tenant2-region1
          {.constraintValue = 4.58 - 1.0,
           .penaltyValue = (4.58 - 1.0) * kNormTenant2NoRoundUp},
          // tenant1-region2
      ...
      delta1ExpectedInfo.goalValue =
          ((4.58 - 3.48) + (4.58 - 3.48) * kNormTenant1NoRoundUp) +
          ((4.58 - 1.0) + (4.58 - 1.0) * kNormTenant2NoRoundUp) +
          ((6.5 - 1.5) + (6.5 - 1.5) * kNormTenant1NoRoundUp) +
          ((6.5 - 1.995) + (6.5 - 1.995) * kNormTenant2NoRoundUp);
      break;
  ...
  VERIFY_CONSTRAINT_COMPONENTS_AND_GOAL_VALUES(
      delta1ExpectedInfo, components, goalExpr, delta1);
  ...
  switch (GetParam()) {
    case interface::CapacityWithGroupPresenceUsageIntent::PER_SCOPE_ITEM: {
      // 2 constraints (one per scope item)
      // trafficObject2 (value 0.4) moves from region2 to region1
      delta2ExpectedInfo.constraintAndPenaltyValues = {
          // Region1 component (not broken)
          {.constraintValue = 4.58 - (3.58 + 1.0),
           .penaltyValue = (4.58 - (3.58 + 1.0)) * kNormPerScopeItemNoRoundUp},
          // Region2 component (broken)
          {.constraintValue = 6.5 - (1.1 + 1.995),
           .penaltyValue = (6.5 - (1.1 + 1.995)) * kNormPerScopeItemNoRoundUp},
      };

      // only region2 broken
      delta2ExpectedInfo.goalValue = (6.5 - (1.1 + 1.995)) +
          (6.5 - (1.1 + 1.995)) * kNormPerScopeItemNoRoundUp;
      break;
    }
    case interface::CapacityWithGroupPresenceUsageIntent::
        PER_GROUP_AND_SCOPE_ITEM: {
      ...
      delta2ExpectedInfo.constraintAndPenaltyValues = {
          // tenant1-region1
          {.constraintValue = 4.58 - 3.58,
           .penaltyValue = (4.58 - 3.58) * kNormTenant1NoRoundUp},
          // tenant2-region1
          {.constraintValue = 4.58 - 1.0,
           .penaltyValue = (4.58 - 1.0) * kNormTenant2NoRoundUp},
          // tenant1-region2: 6.5 - 1.1 = 5.4
          {.constraintValue = 6.5 - 1.1,
           .penaltyValue = (6.5 - 1.1) * kNormTenant1NoRoundUp},
          // tenant2-region2
          {.constraintValue = 6.5 - 1.995,
           .penaltyValue = (6.5 - 1.995) * kNormTenant2NoRoundUp},
      };

      delta2ExpectedInfo.goalValue =
          ((4.58 - 3.58) + (4.58 - 3.58) * kNormTenant1NoRoundUp) +
          ((4.58 - 1.0) + (4.58 - 1.0) * kNormTenant2NoRoundUp) +
          ((6.5 - 1.1) + (6.5 - 1.1) * kNormTenant1NoRoundUp) +
          ((6.5 - 1.995) + (6.5 - 1.995) * kNormTenant2NoRoundUp);
      break;
    }
  }
  VERIFY_CONSTRAINT_COMPONENTS_AND_GOAL_VALUES(
      delta2ExpectedInfo, components, goalExpr, delta2);
}
